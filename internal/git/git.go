...
func CreateTag(ctx context.Context, dir string, tag string) error {
	_, err := runGit(ctx, dir, "tag", "-a", tag, "-m", tag)
	if err != nil {
...
func PushTag(ctx context.Context, dir string, tag string) error {
	_, err := runGit(ctx, dir, "push", "origin", tag)
	if err != nil {
...
func CommitAll(ctx context.Context, dir string, message string) error {
...
func TagTimestamp(ctx context.Context, dir string, tag string) (time.Time, error) {
	out, err := runGit(ctx, dir, "log", "-1", "--format=%aI", tag)
	if err != nil {
		return time.Time{}, fmt.Errorf("get tag timestamp for %s: %w", tag, err)
	}
	t, err := time.Parse(time.RFC3339, strings.TrimSpace(out))
	if err != nil {
		return time.Time{}, fmt.Errorf("parse tag timestamp for %s: %w", tag, err)
	}
	return t, nil
}
